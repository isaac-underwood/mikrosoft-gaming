    ...
    <div class="container top-featured p-3">
        <form action="{{ route('groups.store') }}" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="text" name="name" id="name" class="form-control {{$errors->has('name') ? 'is-invalid' : '' }}" value="{{old('name')}}" placeholder="Enter: Name of Group">
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input type="text" name="description" id="description" class="form-control {{$errors->has('description') ? 'is-invalid' : '' }}" value="{{old('description')}}" placeholder="Enter: Description of Group">
            ...
            <div class="form-group pb-4">
                <label class="text-light-custom">Add Users to Group</label>
                <p><small>You can still add/remove users later.</small></p>
                @include('layouts.groups.users')
                <a href="{{ URL::previous() }}" class="btn btn-danger float-left">< Cancel</a>
                <button type="submit" class="btn btn-success float-right">Create Group <b>+</b></button>
            </div>
        </form>
    </div>
@endsection